  ...
  render(props){
    ...
    if(this.state.realms !== null){
      availableRealms = this.state.realms.map((realm, i) =>{
        if(realm.slug === 'tichondrius'){
          return (<option key={realm.slug} defaultValue="true" value={realm.slug}>{realm.name}</option>);
        }else{
    ...
    return(
      <form className="search-form" onSubmit={this._handleSubmit.bind(this)}>
        <input type="text" defaultValue="Reilen" placeholder="Player or Guild Name" ref={c => this._character = c} className="search-text" />
          <select ref={c => this._realm = c} defaultValue="tichondrius">
            {availableRealms}
          </select>
